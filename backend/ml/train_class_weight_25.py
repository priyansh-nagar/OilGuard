"""
Stage 2B: Small CNN with Training-Only Robustness Augmentation + Class Weight 2.5
...
Experiment: Training-only conservative SAR robustness augmentation with adjusted class weight.
- Horizontal reflection (p=0.50)
- Vertical reflection (p=0.50, independent)
- Global multiplicative intensity gain (p=0.50, uniform [0.90, 1.10], clip [0.0, 1.0])
- Additive zero-mean Gaussian noise (p=0.30, sigma=0.015, clip [0.0, 1.0])
- Positive class weight: 2.5 (vs 1.96 in baseline)

Validation and test receive NO augmentation: only baseline deterministic grayscale + /255.0.
"""
...
OUTPUT_DIR = PROJECT_ROOT / "backend" / "ml" / "outputs" / "class_weight_25"
...
# Training hyperparameters (unchanged from baseline)
BATCH_SIZE = 32
LEARNING_RATE = 0.001
NUM_EPOCHS = 15
...
# Class imbalance: positive-class weight 2.5 (Experiment 2)
CLASS_WEIGHTS = torch.tensor([1.0, 2.5], dtype=torch.float32)

# Checkpoint paths (separate experiment directory)
CHECKPOINT_PATH = OUTPUT_DIR / "checkpoint_latest.pt"
BEST_MODEL_PATH = OUTPUT_DIR / "oil_spill_cnn_best.pt"
FINAL_MODEL_PATH = OUTPUT_DIR / "oil_spill_cnn_class_weight_25.pt"

# Augmentation configuration (recorded for reproducibility)
AUGMENT_CONFIG = {
    "horizontal_flip_prob": 0.50,
    "vertical_flip_prob": 0.50,
    "global_gain_prob": 0.50,
    "global_gain_range": [0.90, 1.10],
    "additive_noise_prob": 0.30,
...
# ============================================================================
# Augmentation Functions (training-only)
# ============================================================================


def apply_training_augmentations(img_array, rng):
    """
    Apply the four stochastic augmentations to a single-channel [0,1] float32 image array.

    Args:
        img_array: numpy array of shape (H, W), float32 in [0.0, 1.0]
        rng: numpy random Generator for reproducibility

    Returns:
        Augmented array of same shape and dtype, clipped to [0.0, 1.0]
    """
    # Horizontal reflection
    if rng.random() < AUGMENT_CONFIG["horizontal_flip_prob"]:
        img_array = np.flip(img_array, axis=1)

    # Vertical reflection (independent)
    if rng.random() < AUGMENT_CONFIG["vertical_flip_prob"]:
        img_array = np.flip(img_array, axis=0)

    # Global multiplicative intensity gain
    if rng.random() < AUGMENT_CONFIG["global_gain_prob"]:
        gain = rng.uniform(*AUGMENT_CONFIG["global_gain_range"])
        img_array = img_array * gain

    # Additive zero-mean Gaussian noise
    if rng.random() < AUGMENT_CONFIG["additive_noise_prob"]:
        noise = rng.normal(0.0, AUGMENT_CONFIG["additive_noise_sigma"], img_array.shape).astype(np.float32)
        img_array = img_array + noise

    # Clip to valid range
    np.clip(img_array, *AUGMENT_CONFIG["clip_range"], out=img_array)

    # Ensure the array is contiguous (positive strides) for torch.from_numpy
...
class OilSpillDataset(Dataset):
    ...
    def __init__(self, image_paths, labels, transform=None, apply_augmentation=False, seed=RANDOM_SEED):
        self.image_paths = image_paths
        self.labels = labels
        self.transform = transform
        self.apply_augmentation = apply_augmentation
        # Per-dataset RNG for reproducible augmentations across epochs
    ...
    def __getitem__(self, idx):
        ...
        # Apply training-only augmentations if enabled
        if self.apply_augmentation:
            img_array = apply_training_augmentations(img_array, self.rng)
...
# ============================================================================
# CNN Model (unchanged from baseline)
# ============================================================================
...
# ============================================================================
# Data Preparation (identical to baseline)
# ============================================================================
...
def prepare_data():
    ...
    Identical to baseline prepare_data() for exact reproducibility.
    """
    print("=" * 80)
...
def main():
    print()
    print("=" * 80)
    print("STAGE 2B: SMALL CNN WITH CLASS WEIGHT 2.5 + ROBUSTNESS AUGMENTATION")
    print("=" * 80)
    print()
    ...
    print()
    print("Augmentation configuration (training only):")
    ...
    # Prepare data (identical seed-42 split as baseline)
    (train_images, train_labels), (val_images, val_labels), (test_images, test_labels) = prepare_data()

    # Create datasets
    # Training: with augmentation
    train_dataset = OilSpillDataset(train_images, train_labels, apply_augmentation=True, seed=RANDOM_SEED)
    # Validation: NO augmentation, baseline deterministic only
    val_dataset = OilSpillDataset(val_images, val_labels, apply_augmentation=False)
    # Test: NO augmentation, baseline deterministic only
    test_dataset = OilSpillDataset(test_images, test_labels, apply_augmentation=False)
    ...
    # Initialize model (fresh, not from any checkpoint)
    ...
    # Loss and optimizer (class weight 2.5)
    criterion = nn.BCEWithLogitsLoss(pos_weight=CLASS_WEIGHTS[1].to(DEVICE))
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)

    # Check for existing checkpoint in THIS experiment's directory only
    ...
    # Save final model with test metrics and experiment configuration
    torch.save(
        {
            "model_state_dict": model.state_dict(),
            "metrics": metrics,
